---
title: Webhooks
description: "Receive real-time notifications when events occur in your Madmail account"
---

## Overview

Webhooks allow you to receive HTTP POST requests to your server when events occur in Madmail, such as when an email is delivered, bounced, or clicked. This enables you to build real-time integrations and automate workflows.
...
<Steps>
  ...
  <Step title="Add webhook in dashboard">
    Go to [Webhooks](https://app.usesend.com/webhooks) in your Madmail dashboard and create a new webhook:
  ...
  <Step title="Verify webhook signatures">
    Always verify webhook signatures to ensure requests are from Madmail. See the [Signature Verification](#signature-verification) section below.
  </Step>
</Steps>
...
If your endpoint doesn't return a 2xx response, Madmail will retry delivery with exponential backoff:
...
<AccordionGroup>
  ...
  <Accordion title="Verify signatures">
    ...
    Madmail and haven't been tampered with.
  </Accordion>
