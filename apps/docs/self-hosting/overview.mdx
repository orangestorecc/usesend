---
title: Self hosting Madmailg
description: "An end-to-end guide on how to self-host Madmail. An open-source sending infrastructure for developers."
---
...
Madmail depends on AWS SES to send emails and SNS to receive email status. Along with that it also depends on Postgres as a database and Redis for queue. Copy the `.env.selfhost.example` file to `.env` and fill in the values.
...
<Tip>
  ...
  Madmail uses GitHub authentication for login.
...
to **Read-only**. Madmail needs the authenticated user's email address to match
...
```
  ...
  Madmail uses Postgres as a database and Redis as a queue. You need to create a new database and add the following environment variables.
...
You can use any platform that supports Docker to host Madmail. If you have any questions drop in the [discord channel](https://discord.gg/gbsvjb9MqV) and we'll try to help you out.
...
In order to send emails, you need to select a region in AWS. Use a region where your users are located / where Madmail is hosted. If you're confused just use `us-east-1`.

You can check available regions [here](https://docs.aws.amazon.com/general/latest/gr/ses.html)

Once you log in to Madmail, it will prompt you add SES configuration.
...
The SMTP proxy server is an optional component that allows applications to send emails through Madmail using standard SMTP protocol instead of the REST API. This is useful for legacy applications, email clients, or any software that needs to send emails via SMTP.
...
services:
  smtp-server:
    ...
    environment:
      ...
      USESEND_BASE_URL: "https://your-usesend-instance.com" # Your Madmail instance URL
...
- **Password**: Your Madmail API key
- **Encryption**: STARTTLS (ports 25, 587, 2587) or SSL/TLS (ports 465, 2465)

<Tip>
  The SMTP proxy forwards all emails to your Madmail instance, so make sure your
  main Madmail application is running and accessible.
</Tip>
...
<Tip>
  A community member shared a short write-up on hosting Madmail with
